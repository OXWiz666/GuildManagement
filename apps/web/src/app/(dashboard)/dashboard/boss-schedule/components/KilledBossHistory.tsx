          ...
          {killedHistory.map((log) => (
            ...
            >
              ...
              <div className="text-[10px] text-zinc-500 space-y-1.5">
                ...
                <p className="flex items-center gap-1.5">👤 <span className="text-zinc-400">Logged By:</span> <span className="text-amber-400 font-semibold">{log.creatorName || "System / Officer"}</span></p>
              ...
              <div className="px-2.5 py-1.5 rounded-lg bg-zinc-950/40 border border-white/[0.04] text-[9.5px] text-zinc-400 flex items-center gap-1.5 font-medium shadow-[0_0_8px_rgba(255,255,255,0.01)] mt-2">
                🎁 <span className="text-zinc-400">Loot:</span> <span className={`${log.lootDrop ? "text-emerald-400 font-extrabold text-[10px]" : "text-zinc-500 italic"}`}>{log.lootDrop || "No loot logged"}</span>
              </div>
