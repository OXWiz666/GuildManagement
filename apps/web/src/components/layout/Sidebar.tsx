...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 h-screen w-[290px] z-50
          bg-[#0a0a0e]/95 backdrop-blur-xl
      ...
      >
        ...
        {user && (
          <div className="px-5 pb-5">
            <p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-[0.12em] px-2 mb-2.5">
              Guild
            </p>
            <GuildSwitcher
              guilds={user.guilds}
          ...
          {navItems.map((item: NavItem) => {
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className={`
                  relative flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl text-[14px] font-medium
...
function GuildSwitcher({
  ...
  if (!active) {
    return (
      <div className="px-3 py-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] text-center">
  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg hover:bg-white/[0.04] transition-all duration-200 text-left cursor-pointer border border-white/[0.04] hover:border-white/[0.10] group"
        aria-expanded={isOpen}
        aria-haspopup="listbox"
      >
        <div className="h-7 w-7 rounded-md bg-white/[0.06] border border-white/[0.10] flex items-center justify-center text-[10px] font-semibold text-white/85 shrink-0 transition-transform duration-300 group-hover:scale-[1.04]">
          {active.guildName[0]}
        </div>
        <div className="min-w-0 flex-1">
          ...
          {guilds.map((guild, i) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-md transition-colors text-left cursor-pointer
                ${i === activeIndex ? "bg-white/[0.06] text-white" : "text-white/50 hover:bg-white/[0.03] hover:text-white/85"}`}
            >
              <div className="h-5 w-5 rounded bg-white/[0.06] flex items-center justify-center text-[9px] font-semibold text-white/80 shrink-0">
                {guild.guildName[0]}
              </div>
